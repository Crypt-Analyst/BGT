...
def send_email_task(subject: str, body: str, from_email: str, to_list: list, reply_to: list | None = None):
    """Send an email. When Celery is available this is a task; otherwise a normal function."""
    try:
        email = EmailMessage(subject=subject, body=body, from_email=from_email, to=to_list, reply_to=reply_to or [])
...
@_task_decorator(ignore_result=True)
def call_openai_chat(messages: list, model: str = "gpt-4o-mini", temperature: float = 0.3) -> dict:
