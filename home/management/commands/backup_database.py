...
class Command(BaseCommand):
    ...
    def backup_database(self):
        ...
        if 'postgresql' in settings.DATABASES['default']['ENGINE']:
            ...
            try:
                subprocess.run([
                    ...
                    db_config.get('NAME', ''),
                ], check=True, env={**os.environ, 'PGPASSWORD': db_config.get('PASSWORD', '')})
        ...
        if media_root.exists():
            ...
            try:
                subprocess.run([
                    'tar', '-czf', str(media_backup), '-C', str(media_root.parent), media_root.name
                ], check=True)
                self.stdout.write(
                ...
                self.stdout.write(
                    self.style.WARNING(f'Media backup failed: {str(e)}')
                )
            ...
            try:
                with open(backup_path, 'r') as f:
                    subprocess.run([
                        ...
                        '--username', db_config.get('USER', ''),
                        db_config.get('NAME', ''),
                    ], stdin=f, check=True, env={**os.environ, 'PGPASSWORD': db_config.get('PASSWORD', '')})
